...
- chatStartersは、この旅程の文脈に応じた「最初に押せる質問」を必ず4つ生成する。以下を厳守:
  - 直行便のみで乗継便が無い場合、乗り継ぎに関する質問は絶対に出さない
  - 国内線（同一国内の移動）の場合: 時間帯・曜日による価格差、繁忙期/閑散期、早割の締切、手荷物条件、便の所要時間など国内旅行で実用的な質問を優先。マイルや乗り継ぎは基本出さない
  - 国際線の場合: 乗り継ぎ空港と待ち時間（乗継便がある場合）、ビザ、時差と所要時間、燃油サーチャージ、預け荷物など国際線特有の質問を優先
  - 往復か片道か（帰国日の有無）に応じた質問にする
  - 質問は各20文字程度の自然な日本語の疑問文。便選び・旅程の判断に役立つもの
