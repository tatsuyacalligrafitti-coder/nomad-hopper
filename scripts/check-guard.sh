...
if [ ! -f "$GUARD" ]; then
  # ${...} と波括弧で囲むこと。囲まないと、bash 3.2 は文字設定がUTF-8のとき
  # 直後の （ を変数名の一部として飲み込み、文字が化ける（2026-08-16 実測）
...
# --- 控えとの見くらべについて（2026-08-16 に外した） --------------------
# もとはリポジトリ内の .claude/hooks/guard.sh と見くらべていた。
# だがリポジトリ内の控えはブランチごとに別の中身になるため、
# 片方のブランチで新しくしても、別のブランチでは古いままで、
# 「本物と控えの中身が違います」が出っぱなしになった。
# 出っぱなしの注意は、本当の異常を埋もれさせる。
#
# 控えは本社フォルダ（~/ai-office/hooks_backup/）に1つだけ置くことにした。
# そちらはブランチの影響を受けず、毎朝 backup_hooks.sh が取り直す。
# ズレは ~/ai-office/check_settings.sh 【5】が見ている。
# リポジトリ内にコピーが残っていないかは、同じく【6】が見ている。
MISMATCH=0
