...
## 完了条件
- [ ] 上記3件の `react-hooks/immutability` エラーが解消される
- [ ] `npm run lint` の残エラーが 19 → 16 になる（減った分が immutability 3件のみであること）
- [ ] `npm run build` が成功する
- [ ] `set-state-in-effect` のエラー件数が7件のまま変化していない（=effect本体に触れていない証拠）
